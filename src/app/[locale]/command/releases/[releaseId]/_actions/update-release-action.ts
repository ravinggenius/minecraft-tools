...
import { Release, RELEASE_ATTRS } from "@/domains/release/schema";
...
const updateRelease =
	(releaseId: Release["id"]): ServerAction =>
	async (data) => {
		"use server";
		...
		try {
			await releaseModel.update(releaseId, result.data);

			revalidatePath(`/${locale}/compendium/releases`);
			revalidatePath(`/${locale}/command/releases`);
			revalidatePath(`/${locale}/command/releases/${releaseId}`);
		} catch (error: unknown) {
